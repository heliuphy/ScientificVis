...
//
// Arguments    : const emxArray_real_T *locations
...
// Return Type  : void
//
void imfill(const boolean_T varargin_1[320000], const emxArray_real_T
            *varargin_2, boolean_T I2[320000])
{
  emxArray_real_T *locationsVar;
  int i2;
  int loop_ub;
  static boolean_T mask[320000];
  double imSize[2];
  emxInit_real_T(&locationsVar, 1);
  checkLocations(varargin_2, locationsVar);
  for (i2 = 0; i2 < 320000; i2++) {
  ...
  for (i2 = 0; i2 < 2; i2++) {
    imSize[i2] = 800.0 + -400.0 * (double)i2;
  }
